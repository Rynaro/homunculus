...
│   │   └── thresholds.toml            # Sensor threshold config (optional)
│   └── git_workflow/
...
```xml
<active_skills>
<skill name="home_monitor" description="Monitor home sensors via MQTT">
  ...skill body...
</skill>
</active_skills>
```
...
**home_monitor** -- Monitors home environment sensors via MQTT.
- Triggers: `sensor`, `temperature`, `humidity`, `device`, etc.
- Auto-activate: `true`
- Tools: `mqtt_subscribe`, `mqtt_publish`, `memory_save`
- Includes sensor topics, thresholds, and alert logic
- Optionally has a companion `thresholds.toml` for configurable limits
...
- [ ] Check temperature sensor (topic: home/sensors/temperature)
  - Alert if below 18C or above 30C
- [ ] Check humidity sensor (topic: home/sensors/humidity)
  - Alert if below 30% or above 80%

## Daily Reminders (weekdays only)
- [ ] Remind about daily standup at 9:00 AM
