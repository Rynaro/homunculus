...
RSpec.describe Homunculus::Skills::Loader do
  ...
  describe "#match_skills" do
    before do
      ...
      create_skill("paludarium",
                   frontmatter: <<~YAML.chomp,
                     name: paludarium
                     description: "Paludarium monitor"
                     triggers: ["paludarium", "humidity", "water temp"]
                     auto_activate: true
                   YAML
                   body: "# Paludarium")
    ...
    it "matches case-insensitively" do
      matched = loader.match_skills(message: "Check the PALUDARIUM humidity")

      expect(matched.size).to eq(1)
      expect(matched.first.name).to eq("paludarium")
    end
