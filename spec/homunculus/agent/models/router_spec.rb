...
RSpec.describe Homunculus::Agent::Models::Router do
  let(:models_config) { TomlRB.load_file("config/models.toml") }
  ...
  describe "#resolve_tier" do
    ...
    context "with skill-based routing" do
      it "maps ruby_dev to coder" do
        tier = router.resolve_tier(skill_name: "ruby_dev")
        expect(tier).to eq(:coder)
      end

      it "maps chinese_companion to workhorse" do
        tier = router.resolve_tier(skill_name: "chinese_companion")
        expect(tier).to eq(:workhorse)
      end

      it "maps paludarium_monitor to whisper" do
        tier = router.resolve_tier(skill_name: "paludarium_monitor")
    ...
    context "priority ordering" do
      it "explicit tier takes priority over skill" do
        tier = router.resolve_tier(tier: :whisper, skill_name: "ruby_dev")
        expect(tier).to eq(:whisper)
      end

      it "skill takes priority over keywords" do
        tier = router.resolve_tier(skill_name: "chinese_companion", user_message: "debug this")
        expect(tier).to eq(:workhorse)
      end
    end
  end
  ...
  describe "#generate" do
    ...
    context "happy path: skill routes to correct tier" do
      it "routes ruby_dev to coder tier and returns a Response" do
        allow(mock_ollama).to receive(:generate).and_return({
        ...
        response = router.generate(messages: messages, skill_name: "ruby_dev")
    ...
    context "cloud escalation tier selection" do
      it "escalates whisper to cloud_fast" do
        ...
        response = router.generate(messages: messages, skill_name: "paludarium_monitor")
